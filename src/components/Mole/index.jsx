...
const isBetween = (min = 0, max = 0, val = 0) => val >= min && val <= max;
...
const Mole = ({
  ...
  const [lookedAd, setLookedAt] = useState(false);
  
//   if (gaze) {
//     const rect = moleContainerRef?.current?.getBoundingClientRect();
    
//     if (isBetween(rect?.left, rect?.right, gaze.x) && isBetween(rect?.top, rect?.bottom, gaze.y)) {
//       setLookedAt(true);
//     } else {
//       setLookedAt(false);
//   }
// }
  ...
  const rect = moleRef?.current?.getBoundingClientRect();
  
  const lookedAt = isBetween(rect?.left, rect?.right, gaze.x) && isBetween(rect?.top, rect?.bottom, gaze.y);
  console.log('lookedAt: ', lookedAt);
